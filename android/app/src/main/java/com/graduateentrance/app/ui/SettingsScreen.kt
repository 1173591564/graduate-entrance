...
@Composable
private fun ConnectionSettingsGroup(todayState: TodayUiState) {
    var baseUrl by remember { mutableStateOf(AppSettings.baseUrl) }
    var token by remember { mutableStateOf(AppSettings.token) }
    ...
    SettingsGroup("账户与连接") {
        Column(
            ...
            Button(
                ...
                Text(if (saved) "已保存，立即生效" else "保存连接设置")
            }
        }
        SettingsRow(
