...
2. Android SDK lives at /home/ubuntu/Android/Sdk (android/local.properties points there). If the
   emulator is missing: `yes | sdkmanager "emulator" "system-images;android-35;google_apis;x86_64"`.
...
4. Create/start the AVD:
   - `echo no | avdmanager create avd -n test35 -k "system-images;android-35;google_apis;x86_64" -d pixel_6`
   - `emulator -avd test35 -gpu swiftshader_indirect -no-snapshot -no-audio &`
...
- Pomodoro specifics: button label is 番茄钟 min(est,25) 分钟; only one pomodoro can run (other
